...
```
data/generate_data.py   -> synthetic but realistic telecom churn dataset (7,000 customers)
model/train.py          -> preprocessing + RandomForest pipeline, CV, evaluation, saves artifact
model/churn_model.joblib -> trained model + metadata (feature order, threshold, train date)
...
The dataset is generated programmatically (`data/generate_data.py`) rather than downloaded, with churn probability built from realistic risk factors (month-to-month contracts, short tenure, high charges, lack of tech support, electronic check payment, etc. all increase churn risk, matching patterns seen in real telecom churn data). This means:
...
# 3. Train the model (produces model/churn_model.joblib + model/metrics.json)
...
curl -X POST http://localhost:8000/predict \
  ...
  -d '{
    ...
    "tenure_months": 3, "phone_service": "Yes", "multiple_lines": "No",
    ...
    "payment_method": "Electronic check", "monthly_charges": 89.5, "total_charges": 268.5
  }'
```

```json
{
  "churn_probability": 0.7788,
  "churn_prediction": "Yes",
  "risk_tier": "High",
  "threshold_used": 0.491
}
```
...
```bash
# Build (make sure the model is trained first, so churn_model.joblib exists)
...
- **Render / Railway** — connect the repo, they auto-detect the Dockerfile.
...
- Add a lightweight frontend (Streamlit) for non-technical users to test predictions
